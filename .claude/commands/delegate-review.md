...
1. Confirm the ocr CLI is installed (command -v ocr; if missing, direct to install: npm install -g @alibaba-group/open-code-review). Run with OCR_NO_UPDATE=1 to suppress a background update check.
2. ocr delegate preview ... to list reviewable files and mode. Report any preview-excluded files (e.g. unsupported_ext) and cover them with the normal code-review five-axis path.
